...
public class CommonAppConfig {
    ...
    //当前app是否是云豹自己的产品
    public static final boolean APP_IS_YUNBAO_SELF = true;
    //直播sdk类型是否由后台控制的
    ...
    public String getOpenId() {
        if(openId==null){
            openId= SpUtil.getInstance().getStringValue("open_id");
        }
        return openId;
    }
    ...
    public void setOpenId(String openId) {
        if(openId!=null){
            this.openId = openId;
            SpUtil.getInstance().setStringValue("open_id",openId);
        }
    }
    ...
    public int getAppIconRes() {
        if (mAppIconRes == 0) {
            mAppIconRes = CommonAppContext.sInstance.getResources().getIdentifier("ic_launcher", "mipmap", "com.ouyezhibo.phonelive");
        }
        return mAppIconRes;
    }
