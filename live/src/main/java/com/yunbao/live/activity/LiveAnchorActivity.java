...
import com.yunbao.live.dialog.LivePublishGoodsDIalogFragment;
import com.yunbao.live.dialog.LiveSelectOpenCouponFragment;
import com.yunbao.live.dialog.OpenDialogFragment;
...
public class LiveAnchorActivity extends LiveActivity implements LiveFunctionClickListener {
    ...
    public static void forward(Context context, int liveSdk) {
    ...
    protected void main() {
        super.main();
        mLiveSDK = getIntent().getIntExtra(Constants.LIVE_SDK, Constants.LIVE_SDK_KSY);
        ...
        if (mLiveSDK == Constants.LIVE_SDK_TX) {
            ...
            mLivePushViewHolder = new LivePushKsyViewHolder(mContext, (ViewGroup) findViewById(R.id.preview_container));
        }
    ...
    public void onClick(int functionID) {
        switch (functionID) {
            ...
            case Constants.LIVE_FUNC_LINK_MIC://连麦
                ...
                break;
            case Constants.LIVE_FUNC_OPEN_GOODS://开启商品
                openShoplinkDialog();
                break;
            case Constants.LIVE_FUNC_COUPON://优惠券
                openShopCouponDialog();

                break;
        }
    }

    private void openShopCouponDialog() {
        LiveSelectOpenCouponFragment selectOpenCouponFragment=new LiveSelectOpenCouponFragment();
        selectOpenCouponFragment.setSocketClient(mSocketClient);
        selectOpenCouponFragment.setOpen(couponIsOpen==1);
        selectOpenCouponFragment.setStreamId(mStream);
        selectOpenCouponFragment.show(getSupportFragmentManager(),"LiveSelectOpenCouponFragment");
    }
    ...
    public void openCouponDialog() {
        CommonHttpUtil.getNeedGetCoupon("view",CommonAppConfig.getInstance().getCity(),mLiveUid, new ShopCallBack() {
            ...
            public void success(ShopResponse shopResponse) {
                List<CouponEntity>data=shopResponse.getCoupons();
                if(ListUtil.haveData(data)){
                    CouponDialog couponDialog=new CouponDialog();
                    couponDialog.setData(shopResponse.getCoupons());
                    couponDialog.setShouldReceiver(false);
    ...
    public void openShoplinkDialog() {
        OpenDialogFragment openDialogFragment=new OpenDialogFragment();
        openDialogFragment.setSocketClient(mSocketClient);
        openDialogFragment.setStream(mStream);
        openDialogFragment.setOpen(shopState==1);
        openDialogFragment.show(getSupportFragmentManager(),"OpenDialogFragment");
    }

    private LiveLinkEntity linkEntity;

    public void setLiveLinkEntity(LiveLinkEntity liveLinkEntity){
        linkEntity=liveLinkEntity;
    }

    public void openShopLinkDialog() {
        LivePublishGoodsDIalogFragment livePublishGoodsDIalogFragment=new LivePublishGoodsDIalogFragment();
        livePublishGoodsDIalogFragment.setLinkEntity(linkEntity);
        livePublishGoodsDIalogFragment.setMessage(mLiveUid,mStream);
        livePublishGoodsDIalogFragment.setSocketClient(mSocketClient);
        livePublishGoodsDIalogFragment.show(getSupportFragmentManager(),"LivePublishGoodsDIalogFragment");
    }
