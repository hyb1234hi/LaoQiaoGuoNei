...
public class LiveRoomViewHolder extends AbsViewHolder implements View.OnClickListener {
    ...
    private ViewGroup rl_shop_icon;
    ...
    public void init() {
        ...
        rl_shop_icon= (ViewGroup) findViewById(R.id.rl_shop_icon);
        ...
        rl_shop_icon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(mContext instanceof LiveAnchorActivity){
                    LiveAnchorActivity liveAnchorActivity= (LiveAnchorActivity) mContext;
                    liveAnchorActivity.openShopLinkDialog();
                }else {
                    API.addShoplinkview(mStream,shopId).subscribe(new DefaultObserver<Boolean>() {
                        @Override
                        public void onNext(Boolean aBoolean) {
                            intentOutWeb(linkUrl);
                        }
                    });
                }
        ...
        btn_coupon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((LiveActivity)mContext).openCouponDialog();
            }
        });
    }
    public void intentOutWeb(String url){
        try {
    ...
    public void setShopLinked(boolean open,String link,String conver){
        if(open){
            if(TextUtils.isEmpty(link)){
                rl_shop_icon.setVisibility(View.INVISIBLE);
            }
            else{
                if(!TextUtils.isEmpty(conver)){
                    ImgLoader.display(conver,img_shop);
                }
                rl_shop_icon.setVisibility(View.VISIBLE);
                linkUrl=link;
            }
        }else{
            rl_shop_icon.setVisibility(View.INVISIBLE);
    ...
    public void setClickable(int isopen,String city){
        setClickable(isopen,city,true);
    }


    public void setClickable(int isopen,String city,boolean click){
        if(isopen==1&&city!=null&&city.equals(CommonAppConfig.getInstance().getCity())){
            btn_coupon.setClickable(true);
            btn_coupon.setVisibility(View.VISIBLE);
            if(click){
                btn_coupon.performClick();
            }
        }else{
